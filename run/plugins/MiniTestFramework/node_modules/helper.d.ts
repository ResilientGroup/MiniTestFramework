...
export class GameTestHelper {
  ...
  assertEntityInventoryContains(pos,entityType,item);
  assertSameBlockState(checkedPos,correctStatePos);
  forEveryBlockInStructure(posConsumer);
  ...
  assertEntityPresent(type,pos1,pos2);
  ...
  assertEntitiesPresent(type,pos,amount: number,radius: number);
  assertAtTickTimeContainerEmpty(delay: number,pos);
  assertItemEntityNotPresent(item,pos,radius: number);
  assertAtTickTimeContainerContains(delay: number,pos,item);
  assertEntityNotTouching(type,x: number,y: number,z: number);
  assertEntityProperty(entity,propertyGetter,propertyName: string,expectedValue);
  assertEntityProperty(entity,predicate,testName: string);
  succeedWhenEntityPresent(type,pos);
  succeedWhenEntityPresent(type,x: number,y: number,z: number);
  assertEntityIsHolding(pos,entityType,item);
  ...
  succeedWhenEntityData(pos,type,entityDataGetter,data);
  spawn(type,pos);
  spawn(type,x: number,y: number,z: number);
  spawn(type,x: number,y: number,z: number);
  spawn(type,pos);
  getHeight(heightmap,x: number,z: number);
  ...
  startSequence();
  walkTo(entity,pos,speed: number);
  ...
  assertGlassPresent(x: number,y: number,z: number);
  ...
  getEntities(type,pos,radius: number);
  ...
  relativeVec(pos);
  placeAt(player,stack,pos,direction);
  onEachTick(runnable);
}
...
export class MiniTestFramework {
  ...
  register(name: string,duration: number,required: boolean,method: (helper: GameTestHelper) => void);
